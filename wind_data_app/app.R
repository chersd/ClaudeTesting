...
server <- function(input, output, session) {
  ...
  output$pollutant_date_selector <- renderUI({
    ...
    site_data$parsed_date <- as.POSIXct(site_data$date_LT_shifted_to_selected_timezone, tz = "PST8")
  ...
  global_y_max <- reactive({
    ...
    sub$parsed_date <- as.POSIXct(sub$date_LT_shifted_to_selected_timezone, tz = "PST8")
  ...
  generate_pollutant_plot <- function() {
    ...
    tryCatch({
      ...
      poll_sub$parsed_date <- as.POSIXct(poll_sub$date_LT_shifted_to_selected_timezone, tz = "PST8")
